...
(defconst dig-browser-version "$Id: dig-browser.el,v 1.11 2003/01/01 07:40:49 
itz Exp $")
...
(defcustom dig-browser-program (cond ((boundp 'dig-program) dig-program) (t 
"dig"))
...
(defvar dig-browser-before-fetch-hook nil
  "Hook for functions to run in Dig Browser mode buffers before dig program 
runs.")

(defvar dig-browser-after-fetch-hook nil
  "Hook for functions to run in Dig Browser mode buffers after dig program 
runs.")

(defvar dig-browser-before-insert-hook nil
  "Hook for functions to run in Dig Browser mode buffers before RRs are 
inserted.")

(defvar dig-browser-after-insert-hook nil
  "Hook for functions to run in Dig Browser mode buffers after RRs are 
inserted.")
...
(defconst dig-browser-mode-map
  (let ((kmap (make-sparse-keymap)))
    ...
    (let ((menu (make-sparse-keymap)))
      ...
      (define-key menu [sort-by-data] '("Sort by Data" . 
dig-browser-sort-by-data))
      (define-key menu [sort-by-type] '("Sort by Type" . 
dig-browser-sort-by-type))
      (define-key menu [sort-by-ttl] '("Sort by TTL" . dig-browser-sort-by-ttl))
      (define-key menu [sort-by-domain] '("Sort by Domain" . 
dig-browser-sort-by-domain))
      (define-key menu [separator-format-2] '("--"))
      (define-key menu [mail-hostmaster] '("Mail Hostmaster" . 
dig-browser-mail-hostmaster))
      (define-key menu [browse-parent] '("Browse Parent" . 
dig-browser-browse-parent))
...
(defun dig-browser-make-rr ()
  "Create a resource record (a list with 5 members) from a line of dig(1) 
output."
...
"*")))
    ...
    (prog1
        (with-current-buffer b
          ...
          (goto-char (point-min))
          (cond
           ((re-search-forward "^;;[ \t]*->>HEADER<<-.* status: \\([A-Z]+\\)" 
nil t)
      ...
      (while rrs
        (let* ((rr (car rrs)) (r0 (nth 0 rr)) (r1 (nth 1 rr))
          ...
          (if (and (string-equal r3 "NS")
              ...
              (let ((l0 (length r0)))
                (put-text-property offset (+ offset l0) 'mouse-face 'highlight 
line)))
      ...
      (let ((i level))
        (while (>= i 0)
          (put-text-property p (point) (dig-browser-gensym i) (nthcdr (- level 
i) intervals))
          (setq i (1- i))))
      (goto-char p)
      (back-to-indentation))))
...
(defun dig-browser-revert (ignore-auto noconfirm)
  ...
  (let ((server (dig-browser-server)))
    ...
    (let* ((d (dig-browser-domain))
      ...
      (if (null rrs) (error "Unable to fetch information for %s from %s" d 
server)
...
\\[dig-browser-goto-domain-at-point] - Jump to a following resource record 
keyed by the domain at point.
\\[dig-browser-mail-hostmaster] - Start an email message to the address from 
the SOA record.
\\[dig-browser-next-subdomain] - Jump to a following NS resource record for a 
subdomain.
\\[dig-browser-browse-other-window] - Open a new window to browse the domain at 
point.
\\[dig-browser-prev-subdomain] - Jump to a preceding NS resource record for a 
subdomain.
\\[dig-browser-up-tree] - Go to the NS RR in the parent domain of the subdomain 
point is on.
\\[dig-browser-browse-parent] - Open a new window to browse the parent of the 
current domain.
\\[dig-browser-collapse] - Collapse the subdomain point is on.
\\[dig-browser-toggle-state] - Assuming the point is on an expandable NS RR, 
...
(defun dig-browser-domain-at-point ()
  "Return the domain name point is on.  Signal error if point is not on a 
domain."
  ...
  (let ((pt (point)) (limit nil) (i 0))
    (if restrict
        (let ((l (dig-browser-interval-level)))
          (save-restriction
            (narrow-to-region (dig-browser-interval-start) 
(dig-browser-interval-end))
            (while (< i n)
...
(dig-browser-interval-end))
            (while (< i n)
              (if (> i 0) (progn (if (bobp) (goto-char (point-max))) 
(forward-line -1)))
              (let ((rr (dig-browser-make-rr)))
                (while (or (not (funcall p rr))
                           (> (dig-browser-interval-level) l))
              ...
              (setq i (1+ i)))))
      (while (< i n)
        (if (> i 0) (progn (if (bobp) (goto-char (point-max))) (forward-line 
-1)))
        (let ((rr (dig-browser-make-rr)))
          (while (not (funcall p rr))
...
(defun dig-browser-hostmaster ()
  ...
  (save-restriction
    ...
    (save-excursion
      (goto-char (point-min))
      (dig-browser-next-rr-satisfying t 1 (lambda (rr) (string-equal "SOA" (nth 
3 rr))))
      ...
      (let* ((bs (bounds-of-thing-at-point 'sexp))
             (s (buffer-substring-no-properties (car bs) (cdr bs))))
        (string-match "\\`\\([^.]+\\)\\.\\(.*[^.]\\)\\.?\\'" s)
        (replace-match "[EMAIL PROTECTED]" nil nil s)))))
...
(defun dig-browser-descendant-p (subdom dom)
  ...
  (let ((lsub (length subdom)) (l (length dom)))
    (and (> lsub (1+ l))
         ...
         (string-equal (downcase dom) (downcase (substring subdom (- lsub 
l)))))))
    ...
    (if (null servers)
      ...
      (let* ((master (dig-browser-fetch-master domain))
             ...
             (server (completing-read (format "Server [%s]: " default)
                                      servers nil t nil 'dig-browser-history 
default)))
        (list domain server)))))
...
Expanded subdomains are temporarily hidden so they aren't affected."
  ...
  (let ((beg (progn (goto-char (dig-browser-interval-start)) (forward-line 1) 
(point)))
...
(defun dig-browser-make-data-key (rr)
  ...
  (let ((data (car (split-string (nth 4 rr)))))
    (cond
     ((string-match
       "\\`\\([0-9]+\\)\\.\\([0-9]+\\)\\.\\([0-9]+\\)\\.\\([0-9]+\\)\\'" data) 
;ip address, format to fixed width
     ...
     ((string-match 
"\\`\\(\\.\\|\\([A-Za-z0-9]\\([-/A-Za-z0-9]*[A-Za-z0-9]\\)?\\.\\)+\\)\\'" data) 
; if a domain, reverse it
      (mapconcat 'identity (nreverse (split-string data "\\.")) "."))
     (t data))))                        ;otherwise just leave it alone

(defun dig-browser-ip-to-arpa (ip)
  "Given an IPv4 address in octet-quad form (as a string), return its arpa 
domain."
    ...
    (if (= (buffer-size) 0)
        (progn
          ...
          (let ((rrs (dig-browser-fetch-zone domain server)))
            (if (null rrs) (error "Unable to fetch information for %s from %s" 
domain server))
            (progn
              (run-hooks 'dig-browser-after-fetch-hook)
              (dig-browser-mode)
  ...
  (let ((d (dig-browser-domain-at-point)))
    (cond
     ((> n 0)
      ...
      (dig-browser-next-rr-satisfying nil n (lambda (rr) (string-equal d (nth 0 
rr))))
      (back-to-indentation))     
     ((< n 0)
      (if (eq last-command 'dig-browser-goto-domain-at-point)
          (progn (if (bobp) (goto-char (point-max))) (forward-line -1)))
      (dig-browser-prev-rr-satisfying nil (- n) (lambda (rr) (string-equal d 
(nth 0 rr))))
      (back-to-indentation))
     (t nil))))
...
(defun dig-browser-browse-parent (domain server)
  ...
  (interactive (dig-browser-get-args (dig-browser-parent-domain 
(dig-browser-domain))))
...
(defun dig-browser-collapse ()
  ...
  (save-restriction
    (narrow-to-region (1- (dig-browser-interval-start)) (1- 
(dig-browser-interval-end)))
